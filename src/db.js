...
export async function migrate() {
  ...
  // 历史领取人一次性回填直连名单：本次更新之前领取过奖励的客户（含链接快发的领取人）
  // 也要能在统一选人名单里被直接找到并定向发放。settings 打点保证只跑一次——
  // 不能每次启动重跑：操作员手动移出名单的人会在下次重启被复活
  const [bf] = await pool.query(`SELECT v FROM settings WHERE k = 'direct_backfill_done'`);
  if (!bf.length) {
    await pool.query(
      `INSERT INTO direct_customers (openid, source, last_claim_at)
       SELECT claimer_openid, 'claim', MAX(updated_at)
         FROM transfers
        WHERE claimer_openid IS NOT NULL AND claimer_openid <> ''
        GROUP BY claimer_openid
